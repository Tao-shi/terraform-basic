...
with open(f"{output_dir}/scripts/route-tables/{res_type}s.json") as f:
    data = json.load(f)
...
for rts in data["RouteTables"]:
    for rt in rts["Associations"]:
        rt_id = rts["RouteTableId"]
        vpc_id = rts.get("VpcId", "null")

        # Prefer tag "Name" for tf_name, fallback to instance ID
        name = get_tag(rts.get("Tags", []), "Name") or rt_id
        tf_name = name.replace("-", "_").replace(" ", "_").lower()

        # Create Terraform block
        tf_block = f'''
resource "aws_route_table" "{tf_name}" {{
  vpc_id = "{vpc_id}"
}}
'''
    ...
    import_result = subprocess.run(["terraform", f"-chdir={output_dir}", "import", f"aws_route_table.{tf_name}", rt_id, "-no-color"], capture_output=True, text=True)
    if import_result.stderr == '':
       print(f"Import of aws_route_table.{tf_name} {rt_id} successful!")
       import_count += 1
    else:
       print(f"Import of aws_route_table.{tf_name} {rt_id} failed!")
       print(import_result.stderr)
    
    print("---------------------------------------------------------------------------------------------------")

    # Unimport resources (optional)
    # subprocess.run(["terraform", f"-chdir={output_dir}", "state", "rm", f"aws_route_table.{tf_name}"])

    # Use state show command to build resource block
    if import_result.returncode != 1 :
        build_result = subprocess.run(["terraform", f"-chdir={output_dir}", "state", "show", f"aws_route_table.{tf_name}", "-no-color"], capture_output=True, text=True)
        if build_result.returncode == 0:
            ...
            print(f"Error building resource block for aws_route_table.{tf_name} {rt_id}!")
