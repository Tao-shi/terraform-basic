...
'''
    ...
    if import_result.stderr == '':
       print(f"Import of aws_vpc_endpoint.{tf_name} {vpce_id} successful!")
       import_count += 1
    else:
       print(f"Import of aws_vpc_endpoint.{tf_name} {vpce_id} failed!")
    ...
    if import_result.returncode != 1 :
        build_result = subprocess.run(["terraform", f"-chdir={output_dir}", "state", "show", f"aws_vpc_endpoint.{tf_name}", "-no-color"], capture_output=True, text=True)
