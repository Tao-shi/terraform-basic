...
with open(f"{output_dir}/scripts/vpcs/{res_type}s.json") as f:
    data = json.load(f)
...
for vpc in data["Vpcs"]:
    ...
    name = get_tag(vpc.get("Tags", []), "Name") or vpc_id
    tf_name = name.replace("-", "_").replace(" ", "_").lower()

    # Create Terraform block
    tf_block = f'''
resource "aws_vpc" "{tf_name}" {{
...
'''
    ...
    import_result = subprocess.run(["terraform", f"-chdir={output_dir}", "import", f"aws_vpc.{tf_name}", vpc_id, "-no-color"], capture_output=True, text=True)
    if import_result.stderr == '':
       print(f"Import of aws_vpc.{tf_name} {vpc_id} successful!")
       import_count += 1
    else:
       print(f"Import of aws_vpc.{tf_name} {vpc_id} failed!")
    ...
    # subprocess.run(["terraform", f"-chdir={output_dir}", "state", "rm", f"aws_vpc.{tf_name}"])

    # Use state show command to build resource block
    if import_result.returncode != 1 :
        build_result = subprocess.run(["terraform", f"-chdir={output_dir}", "state", "show", f"aws_vpc.{tf_name}", "-no-color"], capture_output=True, text=True)
        if build_result.returncode == 0:
            ...
            print(f"Error building resource block for aws_vpc.{tf_name} {vpc_id}!")
