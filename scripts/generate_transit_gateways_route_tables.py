...
with open(f"{output_dir}/scripts/transit-gateway-route-tables/{res_type}s.json") as f:
    data = json.load(f)
...
'''
    ...
    import_result = subprocess.run(["terraform", f"-chdir={output_dir}", "import", f"aws_ec2_transit_gateway_route_table.{tf_name}", tgw_rt_id, "-no-color"], capture_output=True, text=True)
    if import_result.stderr == '':
       print(f"Import of aws_ec2_transit_gateway_route_table.{tf_name} {tgw_rt_id} successful!")
       import_count += 1
    else:
       print(f"Import of aws_ec2_transit_gateway_route_table.{tf_name} {tgw_rt_id} failed!")
    ...
    if import_result.returncode != 1 :
        build_result = subprocess.run(["terraform", f"-chdir={output_dir}", "state", "show", f"aws_ec2_transit_gateway_route_table.{tf_name}", "-no-color"], capture_output=True, text=True)
        if build_result.returncode == 0:
            ...
            print(f"Error building resource block for aws_ec2_transit_gateway_route_table.{tf_name} {tgw_rt_id}!")
